...
int main(void)
{
	...
	printf("\ncha zhao qing kuang:\n");
	if((fp = fopen("sp.dat", "rb")) == NULL) //打开二进制文件sp.dat
	{
		printf("can not open file!\n");
		exit(0);
	}
	
	/* 以下四条语句的功能：从文件sp.dat中读数据 */	
	a = fread(pinming, 20, 1, fp); 
	...
	while (feof(fp) == 0)//判断文件是否结束,为0表示还没有结束
	{
		...
		if ((a == 1) && (b == 1) && (c == 1) && (d == 1))
		{
			if (strcmp(pinming, FindName) == 0)//比较sp.dat中的信息与输入的信息是否相同
			{
