...
void input(struct stu *pstu, int n)
{
	...
	for (i = 0; i < n; i++)
	{
		...
		scanf("%s %s %d", pstu->name, pstu->num, &pstu->score); //输入学生数据
		/* 以下while循环的作用：输入的成绩不在0 ~ 100之间，重新输入 */
		while (pstu->score > 100 || pstu->score <0) 
		{
			printf("           error score! input again!\n");
			printf("student %d: ", i + 1);
			scanf("%s %s %d", pstu->name, pstu->num, &pstu->score);
		}
		pstu++; //指针变量pstu增加1，pstu指向下一个元素
	}
}
...
void output(struct stu *pstu, int n)
{
	...
	for (i = 0; i < n; i++)
	{
		if (pstu->score < 60) //有不及格的课程
		{
			printf("%s/%s,%d   ", pstu->num, pstu->name, pstu->score);
		}
		pstu++; //指针变量pstu增加1，pstu指向下一个元素
	}
}
