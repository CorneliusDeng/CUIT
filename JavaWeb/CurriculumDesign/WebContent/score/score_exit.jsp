...
<body>
	<center>									 
		<form name="frm" method="post" action="<%=request.getContextPath()%>/ScoreAction">
		...
		<input type="hidden" name="cID" value="">
			...
			<tr>
				<td width="60%" align="center" ><input type="button" name="saveBT" value="保存" onclick="check()"></td>
			</tr>
...
{ 
	...
	var url="<%=request.getContextPath()%>/ScoreAction?Action=GetGradesByAjax&major="+major;
...
function getCourse() 
{
    ...
    var url="<%=request.getContextPath()%>/ScoreAction?Action=GetCoursesByAjax&major="+major+"&grade="+grade;
...
{ 
	...
	{// XMLHttpRequest对象未创建
  	...
  	frm.cID.value=courseID;
    var url="<%=request.getContextPath()%>/ScoreAction?Action=GetCourseAllScoresByAjax&courseID="+courseID;
...
{ 
    ...
    if (scores.length != 0) 
    {
        ...
        {
            var score = scores[i];
			html+="<tr align='center'><td>"+(i+1)+"<input type='hidden' name='stID' value='"+score.childNodes[3].childNodes[0].nodeValue+"'></td>"+
			"<td>"+score.childNodes[0].childNodes[0].nodeValue+"</td>"+
			"<td>"+score.childNodes[1].childNodes[0].nodeValue+"</td>"+
			"<td><input type='text' name='score' value='"+score.childNodes[2].childNodes[0].nodeValue+"'></td></tr>";
        }
	   	html+="</table>" ;
