<%@ page contentType="text/html;charset=UTF-8" import="student.*"%>
<%
	...
	<body>
		<center>									 
			...
			<input type="hidden" name="id" value="<%=student.getStID() %>">
				<table border=1>
					<tr class="td_header">
						<td colspan="2"> 学生信息修改，<span class="star">*</span>为必填项</td>
					</tr>
					<tr>
						<td>学号</td>
						<td><input type="text" name="code" value="<%=student.getStCode() %>" size="20" maxlength="5" onchange="checkCode();"><span class="star">*</span>5位字符数字<span id="codeSpan"></span></td>
					</tr>
					<tr>
						<td>姓名</td>
						<td><input type="text" name="name" value="<%=student.getStName() %>" size="20" maxlength="20"><span class="star">*</span></td>
					</tr>
					<tr>
						<td>性别</td>
						<td><input type="radio" name="sex" value="0" <%=student.getSex()==0?"checked":"" %>>男<input type="radio" name="sex" value="1" <%=student.getSex()==1?"checked":"" %>>女<span class="star">*</span></td>
					</tr>
					<tr>
						<td>年级</td>
						<td><input type="text" name="grade" value="<%=student.getGrade() %>" size="20" maxlength="4"><span class="star">*</span>4位整数</td>
					</tr>
					<tr>
						<td>专业</td>
				   	...
				   	<tr>
				   		<td>备注</td>
				   		<td><TEXTAREA name="detail"  rows="2" cols="20" onpropertychange="if(this.value.length>200) {this.value=value.substr(0,200);alert('描述最多20个字符!');}"><%=student.getDetail()==null?"":student.getDetail() %></TEXTAREA>200个字符以内</td>
				   	</tr>
				   	<tr>
				   		<td colspan="2" align="center"><input type="reset"  value="重填" ><input type="button"  value="提交" onclick="check();"></td>
				   	</tr>
				</table>   
			</form>
		</center>
