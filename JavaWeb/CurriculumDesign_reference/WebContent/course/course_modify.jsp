...
<body>
	...
	course = beanDB.select(courseId);
	%> 
   	<form name="frm" method="post" action="<%=path %>/CourseModifyAction">
    <table weight="100%" border=1 align="center">
        <tr>
            <td colspan="2">修改课程信息，<font color="FF2B21">*</font>为必填项</td>
        </tr>
        <tr>
            <td>名称<input type="hidden" name="courseId" value="<%=courseId %>"></td>
            <td>
            	<input type="text" name="name" value="<%=course.getName() %>" onchange="checkName();">
        ...
        <tr>
            ...
            <td>
                <select name="type" id="properties">
                    <option value="1" <%=course.getType()==1?"selected":"" %>>公共必修课</option>
                    <option value="2" <%=course.getType()==2?"selected":"" %>>公共选修课</option>
                    <option value="3" <%=course.getType()==3?"selected":"" %>>专业基础课</option>
                    <option value="4" <%=course.getType()==4?"selected":"" %>>专业必修课</option>
                    <option value="5" <%=course.getType()==5?"selected":"" %>>专业选修课</option>
                </select>
        ...
        <tr>
            <td>学分</td>
            <td>
                <input type="text" name="credit" value="<%=course.getCredit() %>">
        ...
        <tr>
            <td>开设年级</td>
            <td>
                <input type="text" name="grade" value="<%=course.getGrade() %>">
        ...
        <tr>
            <td>开设专业</td>
            <td>
                <input type="radio" checked="checked" id="software" name="major" value="1" <%=course.getMajor()==1?"checked":"" %>><label for="software">软件工程</abel>
                    <input type="radio" id="Spatial Information" name="major" value="2" <%=course.getMajor()==2?"checked":"" %>><label for="Spatial Information">空间信息</abel>
                        <font color="FF2B21">*</font>
            </td>
        </tr>
        <tr>
            <td>备注</td>
            <td>
                <textarea style="overflow:scroll; overflow-x:hidden; height:30px; width: 200px;" cols="2" rows="20" name="detail"><%=course.getDetail()==null?"":course.getDetail() %></textarea>
                200个字符以内
            </td>
        </tr>
        <tr>
