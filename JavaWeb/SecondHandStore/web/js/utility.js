...
//获取商品信息
function getCommodityInfo(commodityID){
	$.ajax({
		type:"POST",
		dataType:"json",
		url:"/commodity/CommodityGetAction",
...
//获取待发布商品的信息
function getTemCommodityInfo(temcommodityID){
	$.ajax({
		type:"POST",
		dataType:"json",
		url:"/temporary/TemCommodityGetAction",
		data:{"temcommodityid" : temcommodityID},
		success: function(data)
		{
			$("#temid").val(data.temid);
			$("#name").val(data.name);
			$("#userid").val(data.userid);
			$("#description").val(data.description);
			$("#money").val(data.money);
